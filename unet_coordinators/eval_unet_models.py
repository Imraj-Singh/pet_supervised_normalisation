...
kldiv = torch.nn.KLDivLoss(reduction='sum', log_target=False)
...
for model_type in models:
    ...
    for idx in range(len(noisy)):
        ...
        get_normalisation = Normalisation(model_type)
        x_preds_save = []
        ssim_s_r = []
        psnr_s_r = []
        kldiv_s_r = []
        ...
        for s in tqdm(range(n_samples)):
            reference = clean_dataset["reference"][s:s+1,0:1,...].to(config.device)
            if tumour:
            ...
            for r in range(n_realisations):
                ...
                reference_scaled = reference * noisy_dataset["scale_factor"][s,0].to(config.device)
                x_pred = model(osem, norm).detach()

                psnr_r.append(peak_signal_noise_ratio(reference_scaled.squeeze().cpu().numpy(), x_pred.squeeze().cpu().numpy(), data_range=reference_scaled.cpu().numpy().max()))
                ssim_r.append(ssim(reference_scaled.squeeze().cpu().numpy(), x_pred.squeeze().cpu().numpy(), data_range=reference_scaled.cpu().numpy().max()))

                y_pred = LPDForwardFunction2D.apply(x_pred, projector, attn_factors) + contamination_factor[:, None, None]
                kldiv_r.append(kldiv(y_pred, measurements).item())
                if tumour:
                    if background.sum() != 0:
                        background_idx = np.nonzero(background.cpu().numpy())
                        b_bar = x_pred.cpu()[background_idx]
                        b_t = reference_scaled.cpu().numpy()[background_idx]
                        crc_t = []
                        for i in range(3):
                            if tumour_rois[:,[i],:,:].sum() != 0:
                                tumour_roi_idx = np.nonzero(tumour_rois[:,[i],:,:].cpu().numpy())
                                a_bar = x_pred.cpu().numpy()[tumour_roi_idx]
                                a_t = reference_scaled.cpu().numpy()[tumour_roi_idx]
                                crc_t.append((a_bar.mean()/b_bar.numpy().mean() - 1) / (a_t.mean()/b_t.mean() - 1))
                        b_bar_r.append(b_bar)
                        crc_r_t.append(torch.asarray(crc_t))
            if tumour:

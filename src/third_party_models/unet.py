"""
Taken from Dival:
https://jleuschn.github.io/docs.dival/dival.reconstructors.networks.unet.html
"""
...
def get_unet_model(in_ch=1, out_ch=1, scales=5, skip=4,
                   channels=(32, 32, 64, 64, 128, 128), use_sigmoid=False,
                   use_norm=True):
    assert (1 <= scales <= 6)
...
class UNet(nn.Module):
    ...
    def forward(self, x0, norm):
        xs = [self.inc(x0/norm[:,None,None,None]), ]
        for i in range(self.scales - 1):
            xs.append(self.down[i](xs[-1]))
        x = xs[-1]
        for i in range(self.scales - 1):
            x = self.up[i](x, xs[-2 - i])
        return torch.sigmoid(self.outc(x)*norm[:,None,None,None]) if self.use_sigmoid else self.outc(x)
